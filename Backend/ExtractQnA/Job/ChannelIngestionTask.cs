...
namespace ExtractQnA.Clients
{
    ...
    {   
        ...
        public async Task RunImplAsync() {
            ...
            foreach (string channelFile in channels) {
                Channel channel = Utils.Utils.ReadChannel(channelFile);

                await this.MakeRequest(channel);
            }
        }

        private async Task MakeRequest(Channel channel) {
...
"\n important, don't forget " + output_tag + ". only useful answers to the thread should be returned, return as less replies as possible, if you find nothing of importance to help other people just return " + output_tag + ".\n\n";
            ...
            // save to file
            string filePath = channel.channelName + "saved.json";
